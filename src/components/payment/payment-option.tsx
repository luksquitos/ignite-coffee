...
export function PaymentOption({ icon: IconComponent, name, onSelect, isActive }: PaymentOptionProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`flex flex-1 gap-3 rounded-md p-4 transition-colors hover:cursor-pointer ${isActive ? 'bg-purple-light border-1 border-purple' : 'bg-base-button hover:bg-base-hover '}`}
    >
